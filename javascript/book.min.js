...
try{console&&console.error&&console.error.apply(console,arguments)}catch(b){}this.logForPad(String.format.apply(this,arguments),"red")}},print:function(){var b=String.format.apply(this,arguments),c=(new Date).format("hh:mm:ss.S");this.debug("[%s] %s",c,b)}}}),VERSION_NAME="6.4.9",BUILD_DATE="2024082301",BookType={normal_book:0,single_book:1,slide_book:3,singleSlide_book:6,catalog_book:7,singleCatalog_book:8},normalBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},
...
getRestrictToken:function(){try{if(!bookConfig.isEncryptionBook||!window.bookId)return"";var b=(new Date).format("yyyy-MM-dd",!0),c=window.localStorage.getItem("restrictToken"+window.bookId+b);return c?c:""}catch(d){}},setRestrictToken:function(b){try{if(!bookConfig.isEncryptionBook||!window.bookId)return"";var c=(new Date).format("yyyy-MM-dd",!0);window.localStorage.setItem("restrictToken"+window.bookId+c,b)}catch(d){}},fillBookContent:function(){this.open=!0;this.loading=!1;this.fillBooks("flipBook");
this.fillBooks("singlePageBook");this.fillBooks("catalogBook");this.fillBooks("slideBook");this.fillBooks("singleSlideBook");BookEvent.trigger("fillBookRestrict",["fillBookRestrict",this.open])},fillBooks:function(b){if(b=global[b])b.loading(),b.fillBookContent()},setRestrictPages:function(b){try{for(var c=1;c<=bookConfig.totalPageCount;c++){a:{var d=c;try{for(var e=b.split(";"),f=0;f<e.length;f++){var g=e[f];if(0<=g.indexOf("-")){var h=g.split("-");if(Number.isBetween(d,parseInt(h[0]),parseInt(h[1]))){var k=
!1;break a}}else if(d==parseInt(g)){k=!1;break a}}}catch(l){}k=!0}k?BookRestrict.unLimitPages[c]="restrict":BookRestrict.unLimitPages[c]="open"}}catch(l){}}}}),Page=Class({create:function(b,c,d,e){this._super(b,c,d,e);this.renderPage();this.addFlipShadow()},remove:function(){this.side.remove();this.render&&(this.render.taskPool.stop(),this.renderInterval&&(window.clearInterval(this.renderInterval),this.renderInterval=void 0))},destroy:function(){this.pageBackground&&this.pageBackground.destroy();
this.sideImage=this.side=void 0;this.hasReader&&this.destroySlider()},hasReader:function(){return window.readerConfig&&0<window.readerConfig.pages.length&&window.readerConfig.pages.length>=this.pageIndex&&(this.hasReaderItems()||window.inverseEditor&&window.inverseEditor.length&&window.inverseEditor[this.pageIndex-1]&&window.inverseEditor[this.pageIndex-1].length)?!0:!1},hasReaderItems:function(){return window.readerConfig.pages[this.pageIndex-1]&&(window.readerConfig.pages[this.pageIndex-1].insertPageLink&&
window.readerConfig.pages[this.pageIndex-1].insertPageLink.length||window.readerConfig.pages[this.pageIndex-1].items&&window.readerConfig.pages[this.pageIndex-1].items.length||window.readerConfig.pages[this.pageIndex-1].hasSceneAnime||window.readerConfig.pages[this.pageIndex-1].background&&window.readerConfig.pages[this.pageIndex-1].background.isEnable||window.readerConfig.pages[this.pageIndex-1].camera&&window.readerConfig.pages[this.pageIndex-1].camera.isEnable||window.readerConfig.pages[this.pageIndex-
1].foreground&&window.readerConfig.pages[this.pageIndex-1].foreground.isEnable||window.readerConfig.pages[this.pageIndex-1].subtitle&&window.readerConfig.pages[this.pageIndex-1].subtitle.isEnable)},loading:function(){"restrict"==BookRestrict.isRestrictPage(this.pageIndex)||this.originalImageLoaded||this.removeLoad||(this.loadingImage||(this.loadingImage=$("<div class='loadingOuter'><img alt='loadingOuter' width='auto' height='auto' class='loadingImage' src='"+bookConfig.pageLoadingUrl+"'/></div>")),
this.sideContent.append(this.loadingImage))},unLoading:function(b){this.hasReader()&&!b||!this.loadingImage||(this.loadingImage.css({display:"none"}),this.loadingImage.remove(),this.loadingImage=null)},restrict:function(){if(!this.restrictImage){this.restrictImage=$("<div class='loadingOuter'></div>");var b=1==this.pageIndex?getMeta("og:image"):bookConfig.pageRestrictUrl;this.restrictImage.css({"background-image":"url("+b+")"});1==this.pageIndex?this.restrictImage.css({"background-size":"contain"}):
...
1],c=0;c<b.length;c++){var d=b[c];if(d)if(d.annotype==PageEditor.AnnoType.ANNO_LINK){if(d.action.actionType!=PageEditor.ActionType.GOTO_PAGE&&d.action.actionType!=PageEditor.ActionType.OPEN_URL&&d.action.actionType!=PageEditor.ActionType.SHOW_ARTICLE&&d.action.actionType!=PageEditor.ActionType.SLIDE_SHOW)return!0;break}else return!0}return!1},getPageAnnos:function(){var b=global.pageEditor;b instanceof Array||(b=b.pageAnnos);if(this.hasInverseEditor()){var c=b;b.length<inverseEditor.length&&(c=inverseEditor);
for(var d=0;d<c.length;d++){b[d]||(b[d]=[]);inverseEditor[d]||(inverseEditor[d]=[]);var e=inverseEditor[d];e&&e.length&&(e=e.map(function(f){return getInverseItem(f,this.width,this.height)}.bind(this)));b[d]=b[d].concat(e);inverseEditor[d]=[]}}return b},hasInverseEditor:function(){return window.inverseEditor&&window.inverseEditor&&window.inverseEditor.length},loadResource:function(){var b=this;this.loadInterval=window.setInterval(function(){PageEditorLoad.isCompleted()&&(window.clearInterval(b.loadInterval),
